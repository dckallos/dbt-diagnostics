...
* Structured symptom grouping and proven root-cause grouping, with each report assembled exactly once.
...
- [ ] NEW-7 -- centralize validated artifact access in one indexed diagnosis context
- [ ] NEW-4 -- make live probing explicit and support artifact-only diagnosis
- [ ] NEW-1 -- preserve DAG edges and resolve disconnects only on reachable paths
- [ ] NEW-2 -- separate observations, typed evidence, and final resolution
- [ ] NEW-3 -- make one canonical JSON report document drive `--json` and terminal output
- [ ] NEW-6 -- verify live Snowflake evidence semantics before release
...
- **OPEN — audit recommendation:** For the first public release, make live
  ...
  tracker.
- **RESOLVED:** The canonical report document is the additive JSON contract; terminal output is derived from it.
...
- [ ] Canonical report-document additions are additive; terminal output derives from that document; CHANGELOG updated.
...
- DAG/path tracing
- Tier-B probe layer
- Canonical report document and terminal projection
...
- [ ] A non-UNION `GETDATE()` mismatch names the responsible expression and source location.
...
**Dependencies:** NEW-5; rewritten #59; NEW-6  
**Release target:** Initial release

PyPI Trusted Publishing uses OIDC rather than a long-lived token, requires `id-token: write`, supports TestPyPI, and is designed to be combined with a protected GitHub environment. ([PyPI Trusted Publishing][4])
...
* [ ] No diagnostic or canonical report-contract changes in this issue.
...
A recognized schema major can span multiple dbt release lines, so the schema-major check alone cannot mean “validated.”
...
**What changed:** Reframed stale-source status as optional historical evidence, removed ungated live scanning, and aligned artifact access with #58 and NEW-7.
...
**What changed:** Made #44 the canonical capture-harness issue, absorbed #12, and aligned naming/provenance with the repository's `real_*` fixture rule.
...
- [ ] NEW-7 -- centralize validated artifact access in indexed views
...
- The canonical report document is the external JSON contract. Its changes remain additive, and terminal output is derived from that document.
...
- [ ] Visibility-limited empty results remain `not_visible_or_missing` or `unverified`.
...
**Labels:** `bug`, `priority: now`
**Parent:** #4
**Dependencies:** #54 semantics; coordinate with NEW-2
**Release target:** Initial release
...
- Parent epic: #4
- Depends on: #54
- Coordinates with: NEW-2 and NEW-3
```

</details>

**What changed:** Distinguished symptom grouping from proven root-cause grouping and moved all group membership into the canonical report document.
---
...
Artifact shape normalization belongs in NEW-7.
...
- Artifact shape validation is tracked by NEW-7
```

</details>

**What changed:** Kept #57 focused on configuration, profile, and dotenv safety; artifact normalization and indexing move to NEW-7.
---
...
Runtime indexing and artifact-context construction are split into NEW-7.
...
- [ ] The registry covers every field exposed by NEW-7's indexed artifact views.
...
- Coordinates with: NEW-7
- Supported by, but not blocked on: #44
```

</details>

**What changed:** Split runtime artifact indexing into NEW-7 and retained only registry/accessor/schema-gate semantics in #58.
---
...
**Parent:** #48
**Dependencies:** #57, #58, NEW-7, NEW-3, and #52 where its fixture is required
**Release target:** Initial release
...
* [ ] No diagnostic behavior or canonical JSON-contract changes in this issue.
...
Tracker searches found no equivalent open or closed issue for edge-preserving lineage, structured evidence resolution, a canonical JSON report document, validated indexed artifact views, explicit live/artifact-only operation, package baseline/security metadata, or bounded pre-release live verification.
...
- canonical lineage fields and terminal projection
...
* Golden canonical report documents for every diagnosis class, followed by terminal projection tests over those documents.
...
* Parent epic: #4
* Depends on: #54, #55, #56, NEW-1
* Blocks: NEW-3 and NEW-6
...
- reports and stable report IDs;
- observations and evidence;
- probe status, identity, and provenance;
- final resolution and confidence;
- symptom groups and root-cause groups;
- group membership and ordering;
- lineage nodes, edges, paths, and truncation;
- manifest diffs;
- offline/live degradation state;
- redaction decisions and omitted-sensitive-field metadata.

## Renderer rules

Terminal rendering may:

- omit detail in non-verbose mode;
- abbreviate identifiers;
- choose headings, spacing, color, and prose presentation;
- display a representative while retaining every member in the document.
...
- generate a fix;
- issue a probe;
- mutate the canonical document.

Every factual terminal claim must map to a field in the canonical document.

## Redaction

Raw matched query text and sensitive literals are excluded by default.
Redaction policy belongs in the canonical-document assembly step so JSON and
terminal output cannot disagree about what is safe to expose.

## Acceptance criteria

- [ ] Diagnosis produces one versioned JSON-compatible canonical document.
- [ ] `--json` serializes that document without recomputing semantics.
- [ ] Terminal rendering accepts only that document or a typed view of it.
- [ ] Grouping and final resolution happen before either output path.
- [ ] Every terminal fact maps to a canonical structured field.
- [ ] `--verbose` changes presentation depth, not diagnostic meaning.
- [ ] Live evidence, failed/unknown states, graph edges, diffs, and group members
      are present in the document.
- [ ] Templates contain formatting logic only.
- [ ] Redaction is identical in JSON and terminal output.
- [ ] Existing JSON keys are not removed or renamed.
- [ ] The additive schema version and output documentation are updated.
- [ ] Parity tests prove that report IDs, groups, verdicts, confidence, and
      evidence displayed in text match the canonical document.

## Test plan

- Golden canonical documents for every diagnosis class.
- Offline and mocked-live canonical-document tests.
- Terminal projection tests over the same fixtures.
- Verbose/non-verbose semantic-equivalence tests.
- Redaction tests.
- Backward-compatibility test against the previous JSON shape.

## Offline behavior

All artifact evidence is represented; absent live evidence is explicitly
unverified.

## Live behavior and cost tier

This issue only assembles and presents evidence already collected. It
introduces no probe.

## Scope and files touched

- canonical report-document model and assembler
- domain-to-document serializers
- CLI output dispatch
- terminal renderer and templates
- JSON and terminal golden tests
- output-contract documentation

## Traceability

- Parent epic: #4
- Depends on: NEW-1, NEW-2, #56
- Blocks: #59 and NEW-6
````
...
- [ ] No diagnostic or canonical report-contract changes.
...
**Parent:** #4
**Dependencies:** #54, #55, #56, NEW-1, NEW-2, NEW-3, NEW-4, #52
...
- [ ] Found, not-visible-or-missing, failed, and unsupported cases remain distinct.
...
```text
validated ArtifactContext
  - ManifestView
  - RunResultsIndex
  - optional CatalogView
  - relation/node indexes
  - timing/query-id indexes
      |
      +-> classifiers
      +-> lineage
      +-> enrichment
...
- Validate top-level artifact and consumed collection shapes once.
- Construct one index per diagnosis.
- Preserve raw entries only behind typed/tolerant views.
- Expose missing, malformed, and unsupported states explicitly.
- Remove repeated result/status scans and private rebuilt indexes.
- Do not perform network or Snowflake access.

## Acceptance criteria

- [ ] Exactly one artifact context is constructed per diagnosis.
- [ ] Every runtime artifact consumer uses the shared views.
- [ ] `DagWalker`, enrichment, and run-identity recovery do not rebuild
      run-results indexes.
- [ ] Wrong-shaped manifest, run-results, catalog, node, source, result, and
      timing inputs degrade or fail at the documented boundary.
- [ ] Direct reads of guarded consumed paths are rejected by a test.
- [ ] Duplicate IDs follow a documented deterministic policy.
- [ ] Existing valid artifact behavior is preserved.
- [ ] Any canonical report-contract change is additive; CHANGELOG updated where
      appropriate.

## Test plan

- Unit tests for each view and index.
- Robustness tests for malformed shapes.
- Contract tests against all committed `real_*` artifacts.
- Performance assertion proving one index construction per diagnosis.
...
No live behavior and no probe.

## Scope and files touched

- new artifact-view/index modules
- `main.py` diagnosis-context construction
- classifiers
- lineage
- enrichment
- run identity
- diffing
- direct-read guard tests

## Traceability

- Parent epic: #48
- Split from: #58
- Blocks: NEW-1, NEW-2, and #59
````
...
| 5 | P0 | #57 — config/profile validation without silent fallback | Initial |
| 6 | P0 | NEW-4 — explicit live and artifact-only mode | Initial |
| 7 | P0 | #56 — structured grouping and single report assembly | Initial |
| 8 | P1 | NEW-7 — validated indexed artifact context | Initial |
| 9 | P1 | #58 — semantic consumed-path registry and schema gate | Initial |
| 10 | P0 | NEW-1 — edge-preserving lineage | Initial |
| 11 | P1 | NEW-2 — typed evidence and final resolution | Initial |
| 12 | P1 | NEW-3 — canonical JSON report document and terminal projection | Initial |
| 13 | P1 | #41 — recognized versus validated dbt versions | Initial |
| 14 | P1 | #52 — real catalog fixture | Initial |
| 15 | P1 | #59 — offline release gates | Initial |
| 16 | P1 | NEW-5 — license/package/security baseline | Initial |
| 17 | P1 | NEW-6 — pre-release live Snowflake verification | Initial |
| 18 | P1 | #33 — TestPyPI and PyPI publication | Initial |
| **CUT** |  | **Publish only after rank 18 is complete and the release candidate is green.** |  |
| 19 | P2 | #43 — `sources.json` freshness evidence | Post-release |
| 20 | P2 | #9 — incremental no-op diagnosis | Post-release |
| 21 | P2 | #8 — expression and UNION type-coercion attribution | Post-release |
| 22 | P2 | #10 — static grain consistency | Post-release |
| 23 | P2 | #44 — cross-version real-artifact matrix | Post-release |
| 24 | P2 | #5 — uniqueness grain tracing | Post-release |
| 25 | P2 | #6 — orphan-key diagnosis | Post-release |
| 26 | P3 | #15 — attested identity spike | Research |
| 27 | P3 | #45 — structured log spike | Research |
| 28 | P3 | #46 — partial-parse spike | Research |
| 29 | P3 | #47 — compatibility watch docs | Research/docs |
---
...
#54 evidence-safe probes ------------------------------+
  |                                                   |
  +--> #56 structured groups / single assembly        |
  |                                                   |
  +-----------------------------+                     |
                                v                     |
NEW-7 indexed artifact views -> NEW-1 lineage graph   |
                                |                     |
                                +----------+----------+
                                           v
                                  NEW-2 evidence/resolution
                                           |
                                           v
                                  NEW-3 canonical JSON document
                                  /                         \
                         `--json` serialize          terminal projection

#57 config/profile safety --> NEW-4 explicit live/artifact-only
                                      |
                                      +------------------------+
                                                               |
#58 semantic schema gate --> #41 version validation --> #59 --+
            |                                                  |
            +-----------------------------------------> #59     |
                                                               |
NEW-7 indexed views ---------------------------------> #59     |
                                                               |
#52 real catalog ----------------------------------------------+
                                                               |
NEW-5 license/package/security --------------------------------+
                                                               |
#54 + #55 + #56 + NEW-1 + NEW-2 + NEW-3 + NEW-4 + #52        |
                              \                                |
...
This path also has two mandatory merge-in prerequisites:

- **NEW-7 → NEW-1**
- **#56 → NEW-2**

The release path is therefore a DAG rather than one purely linear chain.
...
* #55, #57, #58, NEW-7, #41, #52, and NEW-5 can proceed independently.
...
* Offline workflow drafting for #59 can proceed while source fixes land; final golden assertions must wait for NEW-3's canonical report document.
...
1. Edit #55 first and add `priority: now`.
2. Edit #54 and add `priority: now`.
3. Refocus #57 on config/profile safety and add `priority: now`.
4. Rewrite #56 around structured symptom/root-cause groups and canonical report assembly.
5. Split #58: retain semantic registry/schema-gate work in #58 and create NEW-7
   for validated indexed artifact views.
6. Create NEW-1 through NEW-6, plus NEW-7, recording their assigned issue numbers.
7. Rewrite #59 as the offline-only release-gate issue and create NEW-6 as its
   live-verification counterpart.
8. Rewrite #43 and #44; make #44 explicitly absorb #12.
9. Rewrite #41, #33, #5, #6, #8, #9, and #15.
10. Update #8 before closing #28 so all retained timestamp-attribution scope is
    present in the canonical issue.
11. Close #28 as merged into #8.
12. Close #12 as merged into #44.
13. Update #4 and #48 last, substituting assigned issue numbers.
14. Create or identify an `initial public release` milestone and attach every
    P0/P1 implementation issue plus #33.
15. Leave P2/P3 issues outside that milestone.

## Implementation sequence after tracker cleanup

1. #55.
2. #54 and #57.
3. NEW-4, #56, and NEW-7 can proceed in parallel after their prerequisites.
4. #58.
5. NEW-1.
6. NEW-2.
7. NEW-3 and #41.
...
1. **[SPECULATIVE] Does an unattached initial-release milestone already exist?**
   Evidence needed: a complete repository milestone inventory. The fetched issue
   records establish only that no open issue is currently assigned to one.

2. **[SPECULATIVE] Is the first-party schema cache actually committed on the authoritative branch?**
...
3. **[SPECULATIVE] Does `real_schema_change_missing_column_catalog.json` already exist outside the selected snapshot?**
   Evidence needed: the exact fixture file and a non-skipped run of `TestSchemaChangeUsesRealCatalog`.

4. **[SPECULATIVE] Which non-`real_*` fixtures were once guessed rather than deliberately synthetic?**
...
5. **[SPECULATIVE] Is the complete test suite currently green on Python 3.11 and 3.12?**
   Evidence needed: fresh CI or local logs for the full suite, compileall, build, installed-wheel smoke, and offline e2e tests.

6. **[SPECULATIVE] Are branch-protection policies currently applied, and which checks are required live?**
...
7. **[SPECULATIVE] What do the supported Snowflake connector versions return for all relevant `SHOW`, `DESCRIBE`, and query-history rows and exceptions?**
   Evidence needed: recorded, scrubbed rows from NEW-6’s live matrix.

8. **[SPECULATIVE] How do direct and inherited roles affect the package’s intended access conclusions?**
...
9. **[SPECULATIVE] Which license and private vulnerability-reporting channel will the maintainer choose?**
   Evidence needed: maintainer decision recorded in NEW-5.

10. **[SPECULATIVE] Is the PyPI project name reserved and are Trusted Publishers configured for TestPyPI and PyPI?**
   Evidence needed: publisher/environment configuration and a successful TestPyPI workflow run.

11. **[SPECULATIVE] What Tier-B cost ceiling and grain authority are acceptable?**
...
[4]: https://docs.pypi.org/trusted-publishers/using-a-publisher/ "https://docs.pypi.org/trusted-publishers/using-a-publisher/"
