...
The `dbt-diagnostics` package provides you with deterministic workflows that help you once your data pipeline is already broken, similar to American healthcare diagnostics that only help you once you're already sick. I'd recommend that you eat more vegetables so that you're not seeing your doctors so frequently, and I'd recommend dbt packages and tools such as [`dbt-utils`](https://hub.getdbt.com/dbt-labs/dbt_utils/latest/), [`dbt-expectations`](https://hub.getdbt.com/metaplane/dbt_expectations/latest/), [`dbt-project-evaluator`](https://hub.getdbt.com/dbt-labs/dbt_project_evaluator/latest/), [`dbt-audit-helper`](https://hub.getdbt.com/dbt-labs/audit_helper/latest/), [`dbt-checkpoint`](https://github.com/dbt-checkpoint/dbt-checkpoint), [SQLFluff](https://www.sqlfluff.com/), [Elementary](https://docs.elementary-data.com/data-tests/dbt/dbt-package), and the [dbt Fusion engine and VS Code extension](https://docs.getdbt.com/docs/fusion) so that your pipeline doesn't break and you don't need this `dbt-diagnostics` package. You're in the right place if you're already stuck.
